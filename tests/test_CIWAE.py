...
class Test_Model_forward:
    ...
    def test_model_train_output(self, CIWAE, demo_data):
        ...
        assert {"reconstruction_loss", "reg_loss", "loss", "recon_x", "z"} == set(
            out.keys()
        )
...
class Test_CIWAE_Training:
    ...
    def ciwae(self, model_configs, custom_encoder, custom_decoder, request):
        ...
        if alpha < 0.25:
            ...
            return CIWAE(model_configs, encoder=custom_encoder)

        elif 0.5 <= alpha < 0.75:
            return CIWAE(model_configs, decoder=custom_decoder)

        else:
            return CIWAE(model_configs, encoder=custom_encoder, decoder=custom_decoder)
