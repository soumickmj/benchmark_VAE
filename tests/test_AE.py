...
class Test_Model_forward:
    ...
    def test_model_train_output(self, ae, demo_data):
        ...
        assert {"loss", "recon_x", "z"} == set(out.keys())
...
class Test_AE_Training:
    ...
    def ae(self, model_configs, custom_encoder, custom_decoder, request):
        ...
        if alpha < 0.25:
            ...
            return AE(model_configs, encoder=custom_encoder)

        elif 0.5 <= alpha < 0.75:
            return AE(model_configs, decoder=custom_decoder)

        else:
            return AE(model_configs, encoder=custom_encoder, decoder=custom_decoder)
